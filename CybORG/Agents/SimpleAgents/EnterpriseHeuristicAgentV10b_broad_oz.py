"""Optimal heuristic blue agents for CAGE Challenge 4 -- v10b_broad_oz.

Experiment B: Broader OZ threshold=0 exception.
  - ALL hosts in the active OZ subnet get threshold=0 during active phase
    (not just server_host_0). DegradeServices affects all hosts.
  - Phase 1: operational_zone_a user_host_0/1/2 + server_host_0/1/2
  - Phase 2: operational_zone_b user_host_0/1/2 + server_host_0/1/2
...
class EnterpriseHeuristicAgentV10b_BroadOZ:
...
def _is_active_oz_server(hostname: str, phase: int) -> bool:
    """True if hostname is ANY host in the active OZ subnet during its mission phase.

    Exp B: DegradeServices affects ALL hosts in the active OZ, not just
    server_host_0. Immediate Restore (threshold=0) for every host in the
    active operational zone to minimize Impact and service degradation.
    """
...
def make_heuristic_agents_v10b_broad_oz(env) -> dict[str, "EnterpriseHeuristicAgentV10b_BroadOZ"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_BroadOZ] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_BroadOZ(agent_name=agent_name)
        try:
            ag.set_action_info(
