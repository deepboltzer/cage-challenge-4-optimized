"""Optimal heuristic blue agents for CAGE Challenge 4 -- v10b_phase_threshold.

Experiment A: Phase-dependent flag_age threshold.
  - Phase 0 (steps 0-166): threshold=2 (conservative, LWF penalty only -1/step)
  - Phase 1-2 (active phases): threshold=1 (current default)
  - OZ server_host_0 exception (threshold=0) unchanged.
...
class EnterpriseHeuristicAgentV10b_PhaseThreshold:
    ...
    def get_action(
        ...
        for hostname in _sorted_by_priority(proc_alerts, phase):
            ...
            if peer_escalate_t3 or is_critical_oz:
                ...
                threshold = 2  # Exp A: conservative in Phase 0 (LWF penalty only -1/step)
            else:
                threshold = 1
...
def make_heuristic_agents_v10b_phase_threshold(env) -> dict[str, "EnterpriseHeuristicAgentV10b_PhaseThreshold"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_PhaseThreshold] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_PhaseThreshold(agent_name=agent_name)
        try:
            ag.set_action_info(
