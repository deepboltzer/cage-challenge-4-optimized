"""Experiment 5: Restore priority by flag age (oldest first).

Based on v10b. Instead of host priority ordering in P4, sort by flag_age
descending. Hosts with longer-standing flags have had more time for
DegradeServices, so Restoring them first stops the longest-running damage.
"""
...
class EnterpriseHeuristicAgentV10b_OldestFirst:
    ...
    def get_action(
        ...
        #   Default: threshold=1
        # Exp5: Sort proc_alerts by flag_age descending (oldest first)
        proc_hosts_by_age = sorted(
            proc_alerts.keys(),
            key=lambda h: self._step - self._proc_flagged_step.get(h, self._step),
            reverse=True
        )
        for hostname in proc_hosts_by_age:
            if self._busy(hostname):
                continue
...
def make_heuristic_agents_v10b_oldestfirst(env) -> dict[str, "EnterpriseHeuristicAgentV10b_OldestFirst"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_OldestFirst] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_OldestFirst(agent_name=agent_name)
        try:
            ag.set_action_info(
