...
Based on v10b. At steps 165-170 and 332-337 (phase boundaries +/-3),
all proc_flag thresholds drop to 0 for faster response during red repositioning.
"""
...
class EnterpriseHeuristicAgentV10b_PhaseSpike:
    ...
    def get_action(
        ...
        for hostname in _sorted_by_priority(proc_alerts, phase):
            ...
            # Exp3: Drop threshold to 0 near phase boundaries
            near_phase_boundary = (
                165 <= self._step <= 170 or 332 <= self._step <= 337
            )

            if peer_escalate_t3 or is_critical_oz or near_phase_boundary:
                threshold = 0
...
def make_heuristic_agents_v10b_phasespike(env) -> dict[str, "EnterpriseHeuristicAgentV10b_PhaseSpike"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_PhaseSpike] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_PhaseSpike(agent_name=agent_name)
        try:
            ag.set_action_info(
