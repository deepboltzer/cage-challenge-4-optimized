"""Experiment 1: flag_age=0 everywhere (most aggressive Restore).

Based on v10b. All proc_flag thresholds set to 0 (immediate Restore).
This eliminates the 1-step delay but increases false positive Restores.
Purpose: quantify the cost of FP Restores vs benefit of faster red eviction.
"""
...
class EnterpriseHeuristicAgentV10b_FlagAge0:
    ...
    def get_action(
        ...
        for hostname in _sorted_by_priority(proc_alerts, phase):
            ...
            # Exp1: ALL thresholds set to 0 (immediate Restore on any proc_flag)
            threshold = 0
...
def make_heuristic_agents_v10b_flagage0(env) -> dict[str, "EnterpriseHeuristicAgentV10b_FlagAge0"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_FlagAge0] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_FlagAge0(agent_name=agent_name)
        try:
            ag.set_action_info(
