"""Optimal heuristic blue agents for CAGE Challenge 4 -- v10-final.

v10-final: Combined optimization synthesizing ALL experimental findings.
==============================================================================

This agent combines the two independently-validated improvements from sister
experiments (v10 blocking-first + v10b Restore-only) with additional
optimizations from Oracle V3 analysis and ablation data.

Changes vs v9.1 (all evidence-based):
==============================================================================

1. BLOCK/ALLOW AT PRIORITY 1-2 (was priority 2-3 in v9.1)
   Evidence: Oracle V3 biggest win. V1/V2 oracles that blocked after Restore
   scored -1679/-1722; V3 blocking first scored -893.5. Blocking is free,
   instant, prevents red from reaching new hosts.
   Sister experiment: v10 (blocking-first variant)
   Source: docs/swarm_analysis/oracle_architecture.md, Section 3

2. RESTORE-ONLY (Remove eliminated for threat response)
   Evidence: Ablation Restore-only scored -931.7 +/- 221.7 vs full agent
   -1037.2 +/- 610.4 (30 eps, seed 42). Remove 3-step delay + 10% failure
   causes escalation cascades. Restore is definitive.
   Sister experiment: v10b (restore-only variant)
   Source: docs/optimality_analysis.md, Section 4

3. SMART RESTORE THRESHOLDS (flag_age based, phase-aware)
   - Default: flag_age >= 1 (proc flag persists 1 step = likely real red)
   - Impact targets (OZ server_host_0 during active mission): flag_age >= 0
   - T3 escalation (upstream 3+ red hosts): flag_age >= 0
   - High-priority hosts (priority >= 100): flag_age >= 0
   Source: Oracle V3 _is_impact_target logic + red_attack_chain_database.md

4. MAX_DECOYS = 2 (was 3)
   Reduces initial deployment from ~45 to ~30 actions, freeing budget for
   blocking/restoring. 67% failure rate vs 75% -- marginal detection loss.

5. RZ->OZ BLOCK PRIORITY 250 AT PHASE TRANSITION
   Retained from v9.1. Cuts direct lateral movement path first.

Priority order (v10-final):
==============================================================================
P1:  BLOCK per comms_policy (proactive isolation -- free, instant)
P2:  ALLOW per comms_policy (fix stale blocks)
P3:  RESTORE on confirmed red (conn + malfile/proc) -- immediate
P3b: RESTORE on conn-only without decoy coverage
P3c: RESTORE on pure malfile (PrivilegeEscalate signature)
P4:  RESTORE on process flags (smart threshold: flag_age based)
P5:  REDEPLOY DECOYS after Restore
P6:  DEPLOY DECOYS on all hosts (OTService first)
P7:  SLEEP
"""
...
# v10: Reduced from 3 to 2 to free ~15 actions for blocking/restoring.
# With 2 decoys + 1 legitimate service, 67% of blind exploit attempts fail.
MAX_DECOYS = 2

# -- Messaging constants (unchanged from v9) ----------------------------------
...
class EnterpriseHeuristicAgentV10Final:
    """Phase-aware, blocking-first, Restore-only heuristic blue agent for CC4.

    Combines blocking-first priority (from v10) with Restore-only policy (from
    v10b) for optimal heuristic performance. Compatible with both BlueFlatWrapper
    and BlueFlatWrapperV2.
    """
    ...
    def get_action(
        ...
        # Root indicators: malfile without conn/proc = PrivilegeEscalate
        root_indicators: set = {
        ...
        # -- Build outbound message (v9 protocol, unchanged) -----------------
        ...
        # =====================================================================
        # PRIORITY 1: BLOCK per comms_policy (proactive isolation)
        # =====================================================================
        # v10 key change: blocking BEFORE alert response. Oracle V3 showed this
        # is the highest-value action. Sorted by _pair_priority: RZ->OZ first.
        ...
        # =====================================================================
        # PRIORITY 3: RESTORE on confirmed red (conn + malfile/proc)
        # =====================================================================
        ...
        # =====================================================================
        # PRIORITY 3b: RESTORE on conn-only without decoy coverage
        # =====================================================================
        ...
        # =====================================================================
        # PRIORITY 3c: RESTORE on pure malfile (PrivilegeEscalate signature)
        # =====================================================================
        ...
        # =====================================================================
        # PRIORITY 4: RESTORE on process flags (smart threshold)
        # =====================================================================
        # v10-final: Restore-only (no Remove). flag_age threshold:
        #   - Impact targets during active mission: 0 (immediate)
        #   - High-priority hosts (>= 100): 0 (immediate)
        #   - T3 escalation (upstream saturated): 0 (immediate)
        #   - Default: 1 (flag persists 1 step = likely real, not green FP)
        for hostname in _sorted_by_priority(proc_alerts, phase):
            ...
            is_impact = _is_impact_target(hostname, phase)
            is_high_priority = _host_priority(hostname, phase) >= 100

            if peer_escalate_t3 or is_impact or is_high_priority:
                threshold = 0
            else:
                threshold = 1
        ...
        # =====================================================================
        # PRIORITY 5: Re-deploy decoys after Restore
        # =====================================================================
        ...
        # =====================================================================
        # PRIORITY 6: Deploy decoys (initial setup)
        # =====================================================================
        for hostname in self._deploy_hosts:
            if self._busy(hostname):
                continue
    ...
    def _parse_labels(self, labels: list[str], subnet_hosts: dict) -> None:
        ...
        self._restore.clear(); self._decoy.clear()
        self._deploy_hosts = []
...
def _deploy_priority(hostname: str) -> int:
    """Deployment order for decoys. Lower number = higher priority."""
...
def _pair_priority(from_subnet: str, phase: int) -> int:
    ...
    RZ->OZ gets priority 250 at phase transition (highest). Red is likely
    already in RZ from Phase 0 activity, so blocking this path first prevents
    the direct lateral movement to mission-critical hosts.
    """
    if phase == 1:
...
def make_heuristic_v10_final_agents(env) -> dict[str, "EnterpriseHeuristicAgentV10Final"]:
    """Create one v10-final agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10Final] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10Final(agent_name=agent_name)
        try:
            ag.set_action_info(
