"""Optimal heuristic blue agents for CAGE Challenge 4 -- v10b_combined.

Experiment C: Combined A+B.
  A: Phase-dependent threshold (phase 0 -> threshold=2, phase 1-2 -> threshold=1)
  B: Broader OZ threshold=0 for ALL active OZ hosts (not just server_host_0)
...
class EnterpriseHeuristicAgentV10b_Combined:
    ...
    def get_action(
        ...
        for hostname in _sorted_by_priority(proc_alerts, phase):
            ...
            if peer_escalate_t3 or is_critical_oz:
                ...
                threshold = 2  # Exp A: conservative in Phase 0
            else:
                threshold = 1
...
def _is_active_oz_server(hostname: str, phase: int) -> bool:
    ...
    Exp B: ALL hosts in active OZ get immediate Restore (threshold=0).
    """
...
def make_heuristic_agents_v10b_combined(env) -> dict[str, "EnterpriseHeuristicAgentV10b_Combined"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_Combined] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_Combined(agent_name=agent_name)
        try:
            ag.set_action_info(
