"""Heuristic blue agents for CAGE Challenge 4 -- v10.0.

v10.0 change (experimental):
  - Move BLOCK per comms_policy to HIGHEST priority (before alert response)
  - Move ALLOW per comms_policy right after BLOCK
  - Rationale: every step a path remains unblocked, red can attempt new exploits
    through it. Blocking is free, instant, and protects ALL downstream hosts
    simultaneously. Responding to one compromised host is less urgent than
    preventing new compromises across multiple hosts.

  Oracle V3 scored -894 vs heuristic v9.1's -1039. Oracle V3's single biggest
  improvement was blocking at priority 1 -- before any alert response.

New priority order:
  P1:  BLOCK per comms_policy (proactive isolation)
  P2:  ALLOW per comms_policy (fix stale blocks)
  P3:  RESTORE on conn + (malfile OR proc)
  P3b: RESTORE on conn-only without decoy
  P3c: RESTORE on pure malfile (PrivEsc)
  P4:  REMOVE on proc flags
  P5:  RESTORE persistent proc flags
  P6:  REDEPLOY decoys after Restore
  P7:  DEPLOY decoys
  P8:  SLEEP

Everything else is identical to v9.1.
"""
...
class EnterpriseHeuristicAgentV10:
    """v10: Block-first priority ordering.

    Identical to v9.1 except BLOCK and ALLOW are moved to highest priority,
    before any Restore/Remove alert response.
    """
    ...
    def get_action(
        ...
        # ====================================================================
        # v10 REORDER: Block and Allow FIRST, before alert response
        # ====================================================================

        # -- Priority 1 (v10): Block paths per comms_policy ------------------
        ...
        # -- Priority 2 (v10): Allow paths per comms_policy ------------------
        ...
        # -- Priority 3 (v10): Restore on confirmed red ---------------------
        ...
        # -- Priority 3b (v10): Restore on conn-only without decoy ----------
        ...
        # -- Priority 3c (v10): Restore on pure malfile (PrivEsc) -----------
        ...
        # -- Priority 4 (v10): Remove on process flags ----------------------
        ...
        # -- Priority 5 (v10): Restore persistent proc flags -----------------
        for hostname in _sorted_by_priority(proc_alerts, phase):
            if self._busy(hostname):
                continue
        ...
        # -- Priority 6 (v10): Re-deploy decoys after Restore ----------------
...
def make_heuristic_agents_v10(env) -> dict[str, "EnterpriseHeuristicAgentV10"]:
    """Create one v10 agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10(agent_name=agent_name)
        try:
            ag.set_action_info(
