"""Experiment 2: Proactive Restore schedule.

Based on v10b. Adds proactive Restore of high-priority hosts every N steps
(configurable via PROACTIVE_INTERVAL). Inserted between Priority 3 (blocking)
and Priority 4 (alert-driven Restore). Catches invisible DegradeServices.
"""
...
PROACTIVE_INTERVAL = 30  # Restore high-priority host every N steps
...
class EnterpriseHeuristicAgentV10b_Proactive:
    ...
    def __init__(self, agent_name: str = "blue_agent_0"):
        ...
        self._last_proactive_restore: dict[str, int] = {}
    ...
    def get_action(
        ...
        # -- Priority 3.5: Proactive Restore on schedule (Exp2) ---------------
        # Every PROACTIVE_INTERVAL steps, Restore the highest-priority host
        # that hasn't been Restored recently. Catches invisible DegradeServices.
        if self._step % PROACTIVE_INTERVAL == 0 and self._step > 0:
            # Get all restorable hosts sorted by priority
            all_hosts = []
            for sn in self._subnets_in_obs:
                all_hosts.extend(self._subnet_host_list.get(sn, []))
            for hostname in _sorted_by_priority({h: True for h in all_hosts}, phase):
                if self._busy(hostname):
                    continue
                last_proactive = self._last_proactive_restore.get(hostname, -999)
                if self._step - last_proactive < PROACTIVE_INTERVAL:
                    continue
                idx = self._restore.get(hostname)
                if idx is not None and self._valid(idx, mask):
                    self._issue_restore(hostname)
                    self._last_proactive_restore[hostname] = self._step
                    return idx, msg
    ...
    def reset(self) -> None:
        ...
        self._last_proactive_restore.clear()
        self._deploy_hosts = []
...
def make_heuristic_agents_v10b_proactive(env) -> dict[str, "EnterpriseHeuristicAgentV10b_Proactive"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_Proactive] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_Proactive(agent_name=agent_name)
        try:
            ag.set_action_info(
