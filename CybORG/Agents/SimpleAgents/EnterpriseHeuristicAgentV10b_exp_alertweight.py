"""Experiment 4: Alert weighting by signal combination.

Based on v10b. Different thresholds for different alert combinations:
  - conn + malfile -> immediate Restore (both signals = high confidence)
  - conn + proc -> flag_age >= 1 (proc could be FP)
  - proc only -> flag_age >= 2 (likely FP)
  - malfile only -> immediate Restore (PrivEsc = root, highest danger)
"""
...
class EnterpriseHeuristicAgentV10b_AlertWeight:
    ...
    def get_action(
        ...
        # -- Priority 1: Restore on conn + malfile (highest confidence) --------
        for hostname in _sorted_by_priority(conn_alerts, phase):
            if not malfile_alerts.get(hostname):
                continue
        ...
        # -- Priority 1a: Restore on conn + proc (proc may be FP, wait 1 step) -
        for hostname in _sorted_by_priority(conn_alerts, phase):
            if malfile_alerts.get(hostname):
                continue  # already handled in P1
            if not proc_alerts.get(hostname):
                continue
        ...
        for hostname in _sorted_by_priority(proc_alerts, phase):
            ...
            # Exp4: proc-only uses threshold=2 (likely FP), but still
            # allow escalation overrides
            if peer_escalate_t3 or is_critical_oz:
                threshold = 0
            elif peer_escalate_t2 or peer_escalate_compound:
                threshold = 1
            else:
                threshold = 2  # proc-only: wait 2 steps to filter FPs
...
def make_heuristic_agents_v10b_alertweight(env) -> dict[str, "EnterpriseHeuristicAgentV10b_AlertWeight"]:
    """Create one agent per blue team member.  Call AFTER env.reset()."""
    subnet_hosts = getattr(env, "_cached_subnet_hosts", {})
    agents: dict[str, EnterpriseHeuristicAgentV10b_AlertWeight] = {}
    for agent_name in env.possible_agents:
        ag = EnterpriseHeuristicAgentV10b_AlertWeight(agent_name=agent_name)
        try:
            ag.set_action_info(
